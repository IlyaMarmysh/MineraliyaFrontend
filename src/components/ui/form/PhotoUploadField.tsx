...
export const PhotoUploadField = <T extends FieldValues>({
    ...
    const uploadHandler = async (file: File): Promise<string> => {
        try {
            ...
            const presignedResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/generate-presigned-url`, {
                method: 'POST',
                headers: {
            ...
            if (!presignedResponse.ok) {
                ...
                throw new Error(errorData.message || 'Ошибка получения URL для загрузки');
            }
    ...
    return (
        <Controller
            ...
            rules={{
                ...
                validate: (value) => {
                    if (required && !value) return 'Это поле обязательно';
                    return true;
                }
            }}
